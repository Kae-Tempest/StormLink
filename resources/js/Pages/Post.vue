<template>
    ...
    <div class="px-10 pl-[8rem] md:pl-[max(0px,17%)] w-screen">
        <img v-if="post.file" :src="post.file.url" :alt="post.file.url"
            class="w-full h-full max-w-[50%] rounded-[10px] border-2 border-ctp-blue my-5 mx-auto"
            :class="{ 'max-w-[35%]': post.size === 'small' }">

        <div v-if="!post.file" class="text-center p-2 border-2 border-ctp-blue rounded-[10px] mx-auto my-5">
